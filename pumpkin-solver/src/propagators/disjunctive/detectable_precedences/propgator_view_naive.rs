...
#[derive(Clone, Debug)]
pub(crate) struct DetectablePrecedencesViewPropagatorNaive<Var: IntegerVariable> {
...
impl <Var: IntegerVariable + 'static> DetectablePrecedencesViewPropagatorNaive<Var> {
    pub(crate) fn new(arg_tasks: Vec<ArgTaskDisj<Var>>) -> Self {
        ...
        DetectablePrecedencesViewPropagatorNaive {
    ...
    fn detectable_precedences<Var:IntegerVariable + 'static>(tasks: &[TaskDisj<Var>], context: &mut PropagationContextMut, root_bounds: &Vec<(i32, i32)>, i_ect: &Vec<TaskDisj<Var>>, i_lst: &Vec<TaskDisj<Var>>) -> PropagationStatusCP {
        ...
        let mut timeline = TimelineNaive::new(tasks.clone().into(), &assignments);
...
impl <Var> Propagator for DetectablePrecedencesViewPropagatorNaive<Var>
where
