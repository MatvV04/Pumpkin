...
use crate::{basic_types::{HashMap, Inconsistency, PropagationStatusCP}, conjunction, engine::{opaque_domain_event::OpaqueDomainEvent, propagation::{contexts::PropagationContextWithTrailedValues, EnqueueDecision, LocalId, PropagationContext, PropagationContextMut, Propagator, PropagatorInitialisationContext}, reason, Assignments, DomainEvents, IntDomainEvent}, predicate, predicates::PropositionalConjunction, propagators::{ArgTaskDisj, TaskDisj, Timeline}, pumpkin_assert_extreme, pumpkin_assert_moderate, variables::{IntegerVariable, TransformableVariable}};
...
#[derive(Clone, Debug)]
pub(crate) struct DetectablePrecedencesViewPropagator<Var: IntegerVariable> {
...
impl <Var: IntegerVariable + 'static> DetectablePrecedencesViewPropagator<Var> {
    pub(crate) fn new(arg_tasks: Vec<ArgTaskDisj<Var>>) -> Self {
        ...
        DetectablePrecedencesViewPropagator {
    ...
    fn detectable_precedences<Var:IntegerVariable + 'static>(tasks: &[TaskDisj<Var>], context: &mut PropagationContextMut, root_bounds: &Vec<(i32, i32)>, i_ect: &Vec<TaskDisj<Var>>, i_lst: &Vec<TaskDisj<Var>>) -> PropagationStatusCP {
        ...
        let mut timeline = Timeline::new(tasks.clone().into(), &assignments);
       /*  let mut i_lst = tasks.iter().cloned().collect::<Vec<TaskDisj<Var>>>();
        i_lst.sort_by(|a, b| {
            let a_lst = TaskDisj::get_lst(a, &assignments);
            let b_lst = TaskDisj::get_lst(b, &assignments);
            a_lst.cmp(&b_lst)
        });
        let mut i_ect = tasks.iter().cloned().collect::<Vec<TaskDisj<Var>>>();
        i_ect.sort_by(|a, b| {
    ...
    fn get_explanation_left<Var: IntegerVariable + 'static>(task: &TaskDisj<Var>, tasks: &[TaskDisj<Var>], timeline: &Timeline, assignments: &Assignments, root_bounds: &Vec<(i32, i32)>) -> PropositionalConjunction {
        ...
        let est_omega = omega
            .iter()
            .map(|om_task|{
                TaskDisj::get_est(&tasks[om_task.unpack() as usize], assignments)
            }).min().unwrap();
        let p_omega = omega
            .iter()
            .map(|om_task|{
                tasks[om_task.unpack() as usize].duration
            }).sum::<i32>();
        //assert!(est_omega + p_omega == timeline.earliest_completion_time());
        
        let max_lst = omega
            .iter()
            .map(|om_task|{
                TaskDisj::get_lst(&tasks[om_task.unpack() as usize], assignments)
            }).max().unwrap();
        let est_i = TaskDisj::get_est(task, assignments);
        
        let p_i = task.duration;
        let est_i_lift = est_i.min(max_lst - p_i + 1);
        if root_bounds[task.local_id.unpack() as usize].0 <= est_i_lift {
            reason.push(predicate![task.starting_time >= est_i_lift]);
        }
        
        //let ceiled_delta = (est_i + p_i - max_lst) / 2;
        let ubound = est_i + p_i - 1;
        reason.extend(omega.iter().flat_map(|t| {
            let om_task = tasks[t.unpack() as usize].clone();
            let mut om_reason = vec![];
            if root_bounds[t.unpack() as usize].0 <= est_omega {
                om_reason.push(predicate![om_task.starting_time >= est_omega]);
            }
            /*vec![
                predicate![om_task.starting_time >= est_omega],
                predicate![om_task.starting_time <= TaskDisj::get_lst(&om_task, assignments)],
            ]*/
            
            if root_bounds[t.unpack() as usize].1 >= max_lst {
                om_reason.push(predicate![om_task.starting_time <= max_lst]);
            }
            om_reason
        }));
        reason
...
impl <Var> Propagator for DetectablePrecedencesViewPropagator<Var>
where
...
mod tests {
    ...
    use crate::{engine::{propagation::LocalId, test_solver::TestSolver}, propagators::{propagator_view::{get_explanation_left, DetectablePrecedencesViewPropagator}, ArgTaskDisj, TaskDisj, Timeline}};
    ...
    fn test_exp() {
        ...
        let propagator = DetectablePrecedencesViewPropagator::new(argtasks);
        ...
        let reason = get_explanation_left(&tasks[1], &tasks,  &timeline, &assignments, &vec![]);
        println!("{:?}", reason);
    }
    ...
    fn test_prop_example(){
        ...
        let exp = get_explanation_left(&tasks[2], &tasks, &timeline, &solver.assignments, &vec![]);
        println!("{:?}", exp);
        }
}